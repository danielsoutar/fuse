...
typedef struct fcb {
    ...
    // mtime should be updated when changes are made to the FILE.
    // By contrast ctime is updated when changes are made to the FCB
    // atime is updated by read() only (and not if the O_NOATIME flag has been set)
    time_t mtime;   /* time of last modification */
    time_t ctime;   /* time of last change to meta-data (status) */
    time_t atime;   /* time of last access */

    off_t size;     /* size in bytes of the file */

    uuid_t data;    /* the key for the database block that contains the data */
} fcb;
